...
void Convert(const char* strIn, char* strOut, int sourceCodepage, int targetCodepage)
{
	...
	strcpy_s(strOut,512,(char*)pTargetData);
	delete pUnicode;
	delete pTargetData;
}
...
void CppSQLite3DB::open(const char* szFile)
{
	...
	USES_CONVERSION;
#if 0
	CString str = MbcsToUtf8(szFile);
	char* pTemp = T2A(str);
#endif

	char szTemp[256] = {0};
	Convert(szFile, szTemp, CP_ACP, CP_UTF8);

#if _UNICODE
	//int nRet = sqlite3_open(szFile, &mpDB);
	int nRet = sqlite3_open(szTemp, &mpDB);
#else
	int nRet = sqlite3_open((LPCTSTR)str, &mpDB);
#endif
	*/
...
bool CppSQLite3DB::isTableExist(const char* szTable)
{
	char szSQL[128] = {0};
