...
//Add to ReportCtrl
BOOL CReportCtrl::_IsValidColumn(int nColumn) const
{
	return nColumn >= 0 && nColumn < GetColumnCount();
}

CHeaderData* CReportCtrl::GetHeaderData(int nColumn)
{
	if (!HasColumnHeader() || !_IsValidColumn(nColumn) || !GetHeaderCtrl())
		return NULL;
	...
	return (CHeaderData*)hi.lParam;
}

void CReportCtrl::SetHeaderMoveable(int nColumn /* = -1 */, BOOL bMoveable /* = TRUE */)
{
	if (_IsValidColumn(nColumn))
	{
		GetHeaderData(nColumn)->bMoveable = bMoveable;
	}
	else
	{
		for (int i = 0; i < GetColumnCount(); ++i)
		{
			GetHeaderData(i)->bMoveable = bMoveable;
		}
	}
}
